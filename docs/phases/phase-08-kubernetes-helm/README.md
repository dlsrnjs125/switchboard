...
- Pod-scoped Kafka consumer groups so every Distribution replica reconciles each freshness notification into its independent cache and broadcasts it to its own connected SDK sessions.
...
4. two SDK probes pinned to different Distribution Pods both observe version 1, then both apply version 2 after a second Publish;
5. 600 local SDK evaluations remain `false` on version 2 while Distribution rolls;
...
- Pod-scoped Kafka groups replay retained notifications from `earliest` and reconcile against PostgreSQL, but inactive group metadata and notification retention still require explicit broker policies.
