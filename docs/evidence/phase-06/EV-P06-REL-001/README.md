...
- **Git commit:** `d789192cb7d649b1952bedb867aeda6956cdc2dc`
- **Executed at:** 2026-09-21T04:55:12Z
...
- One PostgreSQL container; Toxiproxy cuts every application connection before a publish transaction can commit.
...
- Disk LKG restart includes a valid active file plus an orphaned partial temporary artifact.
...
- [x] Disk replacement fsyncs file content and parent-directory metadata; interrupted temporary artifacts remain inactive.
- [x] Every injected fault is removed during cleanup.

## Observed

- SDK Provider suite: 17 tests, 0 failures.
- Control Plane suite: 26 tests, 0 failures.
...
- The PostgreSQL drill cuts connectivity before commit; it does not force the ambiguous interval after server commit but before client acknowledgement.
